...
test-ci:
ifdef cover
	$(GO) test -run "[^FLAKY]$$" -coverprofile=cover.out ./pkg/puller
else
	$(GO) test -run "[^FLAKY]$$" ./pkg/puller
endif

.PHONY: test-ci-race
test-ci-race:
ifdef cover
	$(GO) test -race -run "[^FLAKY]$$" -coverprofile=cover.out ./pkg/puller
else
	$(GO) test -race -run "[^FLAKY]$$" ./pkg/puller
endif
