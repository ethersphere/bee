...
func Test_UnmarshalBinary(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err:  ErrInvalidVersionHash,
		},
		{ // fork with metadata, where metadata size value is less then metadata content (size 89, want 93)
		...
		{ // fork with vaild metadata (size is 93)
	...
	for _, tc := range tests {
		...
		if tc.err != nil && !errors.Is(err, tc.err) {
			t.Fatalf("expected error marshaling, want %v got %v", tc.err, err)
