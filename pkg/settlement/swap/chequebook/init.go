...
func checkBalance(
	...
	for {
		...
		if balance.Cmp(swapInitialDeposit) < 0 {
			...
			if chainId == 5 {
				logger.Warningf("get your Goerli ETH and Goerli BZZ now via the BZZ exchange at https://bzz.ethswarm.org/?transaction=buy&amount=%d&slippage=30&receiver=%x", new(big.Int).Div(swapInitialDeposit, big.NewInt(10000000000000000)), overlayEthAddress)
			}
			select {
