...
type Service struct {
	...
	peerPriceTableFunc    func(peer swarm.Address) (priceTable []uint64, err error)
...
func (pricer *Service) PeerPriceTable(peer swarm.Address) (priceTable []uint64, err error) {
	if pricer.peerPriceTableFunc != nil {
		return pricer.peerPriceTableFunc(peer)
	}
	return nil, nil
}
