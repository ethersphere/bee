...
func TestMatchesSender(t *testing.T) {
	...
	t.Run("fail to retrieve tx from backend", func(t *testing.T) {
		...
		_, _, err := matcher.Matches(context.Background(), trx, 0, swarm.NewAddress([]byte{}))
	...
	t.Run("sender does not match", func(t *testing.T) {
		...
		matches, _, err := matcher.Matches(context.Background(), trx, 0, swarm.NewAddress([]byte{}))
		if err != nil {
	...
	t.Run("sender matches", func(t *testing.T) {
		...
		senderOverlay := crypto.NewOverlayFromEthereumAddress(signer.addr.Bytes(), 0, trx)

		matches, _, err := matcher.Matches(context.Background(), trx, 0, senderOverlay)
		if err != nil {
			t.Fatalf("expected no err, got %v", err)
		}
