...
func (s *Service) actEncryptionHandler(
	...
	ls := loadsave.New(s.envelopeDownload(true), s.envelopeCache(), requestPipelineFactory(ctx, putter, false, redundancy.NONE), rLevel)
...
func (s *Service) actGrantRevokeHandler(w http.ResponseWriter, r *http.Request) {
	...
	publisher := &s.publicKey
	ls := loadsave.New(s.envelopeDownload(true), s.envelopeCache(), requestPipelineFactory(ctx, putter, false, redundancy.NONE), rLevel)
	gls := loadsave.New(s.envelopeDownload(true), s.envelopeCache(), requestPipelineFactory(ctx, putter, granteeListEncrypt, redundancy.NONE), rLevel)
