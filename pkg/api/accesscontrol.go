...
func (s *Service) actEncryptionHandler(
	...
	ls := loadsave.New(s.storer.Download(&storer.DownloadOpts{Cache: true}), s.storer.Cache(), requestPipelineFactory(ctx, putter, false, redundancy.NONE), redundancy.DefaultLevel)
...
func (s *Service) actGrantRevokeHandler(w http.ResponseWriter, r *http.Request) {
	...
	publisher := &s.publicKey
	ls := loadsave.New(s.storer.Download(&storer.DownloadOpts{Cache: true}), s.storer.Cache(), requestPipelineFactory(ctx, putter, false, redundancy.NONE), redundancy.DefaultLevel)
	gls := loadsave.New(s.storer.Download(&storer.DownloadOpts{Cache: true}), s.storer.Cache(), requestPipelineFactory(ctx, putter, granteeListEncrypt, redundancy.NONE), redundancy.DefaultLevel)
