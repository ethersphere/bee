...
func (s *Service) mountAPI() {
	...
	handle("/pubsub", jsonhttp.MethodHandler{
		"GET": web.ChainHandlers(web.FinalHandlerFunc(s.bpsTopicsHandler)),
	})

	handle("/pubsub/{topic}", jsonhttp.MethodHandler{
		"GET": http.HandlerFunc(s.bpsWsHandler),
	})
