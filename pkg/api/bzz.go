...
func (s *Service) fileUploadHandler(w http.ResponseWriter, r *http.Request, storer storage.Storer, waitFn func() error) {
	...
	if !created {
		...
		if estimatedTotalChunks := requestCalculateNumberOfChunks(r); estimatedTotalChunks > 0 {
			...
			if err != nil {
				...
				jsonhttp.InternalServerError(w, fmt.Sprintf("increment tag: %v", err))
				return
			}
		}
	}
	...
	if err != nil {
		...
		jsonhttp.InternalServerError(w, fmt.Sprintf("create manifest: %v", err))
		return
	}
	...
	if err != nil {
		...
		jsonhttp.InternalServerError(w, fmt.Sprintf("add metadata: %v", err))
		return
	}
	...
	if err != nil {
		...
		jsonhttp.InternalServerError(w, fmt.Sprintf("add file: %v", err))
		return
	}
	...
	if err != nil {
		...
		case errors.Is(err, postage.ErrBucketFull):
			jsonhttp.PaymentRequired(w, "batch is overissued")
		default:
			jsonhttp.InternalServerError(w, fmt.Sprintf("manifest store: %v", err))
		}
		return
	}
...
func (s *Service) downloadHandler(w http.ResponseWriter, r *http.Request, reference swarm.Address, additionalHeaders http.Header, etag bool) {
	...
	if err != nil {
		...
		jsonhttp.InternalServerError(w, fmt.Sprintf("joiner: %v", err))
		return
	}
