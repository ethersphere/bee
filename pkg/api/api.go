...
type server struct {
	...
	chunkPushC      chan chan *pusher.Op
	Options
	http.Handler
	metrics metrics
...
func New(tags *tags.Tags, storer storage.Storer, resolver resolver.Interface, pss pss.Interface, traversalService traversal.Traverser, pinning pinning.Interface, feedFactory feeds.Factory, post postage.Service, postageContract postagecontract.Interface, steward steward.Interface, signer crypto.Signer, logger logging.Logger, tracer *tracing.Tracer, o Options) (Service, <-chan chan *pusher.Op) {
	s := &server{
		...
		steward:         steward,
		chunkPushC:      make(chan chan *pusher.Op),
		signer:          signer,
		Options:         o,
		logger:          logger,
		tracer:          tracer,
...
type pushStamperPutter struct {
	...
	eg      errgroup.Group
	c       chan chan *pusher.Op
	sem     chan struct{}
}

func newPushStamperPutter(s storage.Storer, post postage.Service, signer crypto.Signer, batch []byte, cc chan chan *pusher.Op) (*pushStamperPutter, error) {
