...
func (s *store) Count(key storageV2.Key) (int, error) {
	...
	keys := util.BytesPrefix([]byte(key.Namespace() + forwardSlash))
...
func (s *store) Put(item storageV2.Item) error {
	...
	key := []byte(item.Namespace() + forwardSlash + item.ID())
...
func (s *store) Has(sKey storageV2.Key) (bool, error) {
	...
	key := []byte(sKey.Namespace() + forwardSlash + sKey.ID())
