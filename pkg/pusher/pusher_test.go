...
func TestSendChunkToSyncWithTag(t *testing.T) {
	...
	mtags, p, storer := createPusher(t, triggerPeer, pushSyncService, defaultValidStamp, mock.WithClosestPeer(closestPeer), mock.WithNeighborhoodDepth(0))
	defer storer.Close()
	defer p.Close()
...
func TestSendChunkToPushSyncWithoutTag(t *testing.T) {
	...
	_, p, storer := createPusher(t, triggerPeer, pushSyncService, defaultValidStamp, mock.WithClosestPeer(closestPeer), mock.WithNeighborhoodDepth(0))
	defer storer.Close()
	defer p.Close()
...
func TestSendChunkAndReceiveInvalidReceipt(t *testing.T) {
	...
	_, p, storer := createPusher(t, triggerPeer, pushSyncService, defaultValidStamp, mock.WithClosestPeer(closestPeer))
	defer storer.Close()
	defer p.Close()
...
func TestPusherRetryShallow(t *testing.T) {
	...
	*pusher.RetryCount = 3
	...
	_, ps, storer := createPusher(t, pivotPeer, pushSyncService, defaultValidStamp, mock.WithClosestPeer(closestPeer), mock.WithNeighborhoodDepth(31))
	defer ps.Close()
