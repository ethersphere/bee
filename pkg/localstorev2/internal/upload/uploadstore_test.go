...
func TestIterate(t *testing.T) {
	...
	t.Run("on empty storage does not call the callback fn", func(t *testing.T) {
		upload.Iterate(context.TODO(), ts, nil, func(chunk swarm.Chunk) (bool, error) {
	...
	t.Run("respects start from chunk", func(t *testing.T) {
		...
		putter.Put(context.TODO(), chunk1)
		putter.Put(context.TODO(), chunk2)

		var count int
		upload.Iterate(context.TODO(), ts, chunk1, func(chunk swarm.Chunk) (bool, error) {
			count++
			return false, nil
		})
		upload.Iterate(context.TODO(), ts, chunk2, func(chunk swarm.Chunk) (bool, error) {
			count++
			return false, nil
		})
