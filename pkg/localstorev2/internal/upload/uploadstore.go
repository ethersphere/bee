...
func (u *uploadPutter) Put(ctx context.Context, chunk swarm.Chunk) error {
	...
	if err := chunkstamp.Store(u.s, chunkStampNamespace, chunk); err != nil {
...
func Iterate(ctx context.Context, s internal.Storage, startFrom swarm.Chunk, consumerFn func(chunk swarm.Chunk) (bool, error)) error {
	...
	return s.IndexStore().Iterate(q, func(r storage.Result) (bool, error) {
		pi := r.Entry.(*pushItem)
		chunk, err := s.ChunkStore().Get(ctx, pi.Address) // .GetWithStamp(ctx, pi.Address, pi.BatchID)
		if err != nil {
			return true, err
		}

		stamp, err := chunkstamp.LoadWithBatchID(s, chunkStampNamespace, chunk.Address(), pi.BatchID)
		if err != nil {
			return true, err
		}
