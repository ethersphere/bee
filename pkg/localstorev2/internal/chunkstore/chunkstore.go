...
func (c *chunkStoreWrapper) getStamp(addr swarm.Address) (swarm.Stamp, error) {
	...
	err := c.st.Iterate(storage.Query{
...
func (c *chunkStoreWrapper) readChunk(ctx context.Context, rIdx *retrievalIndexItem) (swarm.Chunk, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("chunk store: failed to read stamp: %w", err)
	}
	if stamp == nil {
	...
	if err != nil {
		return nil, fmt.Errorf("chunk store: failed reading location: %v from sharky: %w", rIdx.Location, err)
	}
...
func (c *chunkStoreWrapper) Get(ctx context.Context, addr swarm.Address) (swarm.Chunk, error) {
	...
	err := c.st.Get(rIdx)
	if err != nil {
		return nil, fmt.Errorf("chunk store: failed reading retrievalIndex: %w", err)
	}
...
func (c *chunkStoreWrapper) Put(ctx context.Context, ch swarm.Chunk) (bool, error) {
	...
	err = func() error {
		...
		err = c.st.Put(&chunkStampItem{Address: ch.Address(), Stamp: ch.Stamp()})
		if err != nil {
	...
	if err != nil && !found {
		_ = c.sharky.Release(context.Background(), loc)
		return false, err
	}
...
func (c *chunkStoreWrapper) Delete(ctx context.Context, addr swarm.Address) error {
	...
	err := c.st.Get(rIdx)
	if err != nil {
		return fmt.Errorf("chunk store: failed to read retrievalIndex: %w", err)
	}
	...
	if rIdx.RefCnt > 0 {
		...
		err = c.st.Put(rIdx)
		if err != nil {
			return fmt.Errorf("chunk store: failed updating retrievalIndex: %w", err)
		}
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("chunk store: failed to release sharky slot: %w", err)
	}

	err = c.st.Delete(rIdx)
	if err != nil {
		return fmt.Errorf("chunk store: failed to delete retrievalIndex: %w", err)
	}

	err = c.removeStamps(rIdx.Address)
	if err != nil {
		return fmt.Errorf("chunk store: failed removing stamps: %w", err)
	}
...
func (c *chunkStoreWrapper) Iterate(ctx context.Context, fn storage.IterateChunkFn) error {
	return c.st.Iterate(storage.Query{
