...
func (p *pinCollectionItem) Unmarshal(buf []byte) error {
	...
	ni.UUID = string(buf[swarm.HashSize : swarm.HashSize+uuidSize])
...
func (p *pinChunkItem) Namespace() string { return p.UUID }
...
func NewCollection(st Storage, root swarm.Address) (PutterCloser, error) {
	...
	collection := &pinCollectionItem{Addr: root, UUID: uuid.NewString()}
