...
func (i *item) Namespace() string {
	return path.Join("chunkStamp", i.address.ByteString())
}
...
// Load returns swarm.Stamp related to the given address and batchID.
// The storage.ErrNoStampsForChunk is returned if no record is found.
func Load(s internal.Storage, namespace string, addr swarm.Address, batchID []byte) (swarm.Stamp, error) {
...
// Delete removes the related stamps from the storage.
func Delete(s internal.Storage, namespace string, addr swarm.Address) error {
	...
	for _, stamp := range stamps {
		errs = multierror.Append(
			errs,
			s.IndexStore().Delete(&item{address: addr, stamp: stamp}),
		)
	}
