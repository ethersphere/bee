...
func TestAccountingAddOriginatedBalance(t *testing.T) {
	...
	f := func(ctx context.Context, peer swarm.Address, amount *big.Int) {
		acc.NotifyRefreshmentSent(peer, big.NewInt(0), big.NewInt(0), 0, 0, nil)
	}
...
func TestAccountingReserve(t *testing.T) {
	...
	_, err = acc.PrepareCredit(peer1Addr, testPaymentThreshold.Uint64()+2*uint64(testRefreshRate)+1, true)
	if err == nil {
...
func TestAccountingReserveAheadOfTime(t *testing.T) {
	...
	_, err = acc.PrepareCredit(peer1Addr, testPaymentThreshold.Uint64(), false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingCallSettlementEarly(t *testing.T) {
	...
	creditAction, err = acc.PrepareCredit(peer1Addr, debt2, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingCallPaymentErrorRetries(t *testing.T) {
	...
	// Credit until near payment threshold
	creditAction, err = acc.PrepareCredit(peer1Addr, 80, true)
	if err != nil {
		t.Fatal(err)
	}
	if err = creditAction.Apply(); err != nil {
...
func debitAndRefresh(t *testing.T, acc *accounting.Accounting, peer1Addr swarm.Address, debitRefresh int64) {
	// Create debt
	debitAction, err := acc.PrepareDebit(peer1Addr, uint64(debitRefresh))
	if err != nil {
		t.Fatal(err)
	}
	err = debitAction.Apply()
