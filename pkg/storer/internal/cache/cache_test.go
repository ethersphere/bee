...
func TestCache(t *testing.T) {
	...
	t.Run("getter", func(t *testing.T) {
		...
		t.Run("get reverse order", func(t *testing.T) {
			...
			for idx, chunk := range slices.Backward(chunks) {
				readChunk, err := c.Getter(st).Get(context.TODO(), chunk.Address())
				if err != nil {
					t.Fatal(err)
				}
				if !readChunk.Equal(chunk) {
					t.Fatalf("incorrect chunk: %s", chunk.Address())
				}
				if idx == 0 {
					// once we access the first entry, the top will change
					verifyCacheState(t, st.IndexStore(), c, chunks[9].Address(), chunk.Address(), 10)
				} else {
					verifyCacheState(t, st.IndexStore(), c, chunks[0].Address(), chunk.Address(), 10)
				}
				newOrder = append(newOrder, chunk)
			}
