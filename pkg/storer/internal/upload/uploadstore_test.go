...
func TestChunkPutter(t *testing.T) {
	...
	t.Run("iterate all", func(t *testing.T) {
		count := 0
		err := upload.IterateAll(ts.ReadOnly().IndexStore(), func(addr swarm.Address, synced bool) (bool, error) {
			count++
			if synced {
				t.Fatal("expected synced to be false")
			}
			has, err := ts.ReadOnly().ChunkStore().Has(context.Background(), addr)
			if err != nil {
...
func TestStampIndexHandling(t *testing.T) {
	...
	t.Run("put existing chunk with newer batch timestamp", func(t *testing.T) {
		...
		if err := put(t, ts, putter, chunk2); err != nil {
			t.Fatalf("Put(...): unexpected error: %v", err)
		}
	})
}
