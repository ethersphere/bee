...
func TestSalud(t *testing.T) {
	...
	service := salud.New(statusM, topM, reserve, log.Noop, ssmock.NewSubscriber(true), "full", 0, 0.8, 0.8)
...
func TestSubToRadius(t *testing.T) {
	...
	service := salud.New(&statusMock{make(map[string]peer)}, topM, mockstorer.NewReserve(), log.Noop, ssmock.NewSubscriber(true), "full", 0, 0.8, 0.8)
