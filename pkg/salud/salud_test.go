...
func TestSalud(t *testing.T) {
	...
	service := salud.New(statusM, topM, reserve, log.Noop, stabilmock.NewSubscriber(true), "full", 0.8, 0.8, nil)
...
func TestSubToRadius(t *testing.T) {
	...
	service := salud.New(&statusMock{make(map[string]peer)}, topM, mockstorer.NewReserve(), log.Noop, stabilmock.NewSubscriber(true), "full", 0.8, 0.8, nil)
