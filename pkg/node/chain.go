...
func InitChain(
	...
	if chainEnabled {
		...
		if err != nil {
			logger.Info("could not connect to backend; in a swap-enabled network a working blockchain node (for xdai network in production, chiado in testnet) is required; check your node or specify another node using --swap-endpoint.", "backend_endpoint", endpoint)
