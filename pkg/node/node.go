...
func NewBee(interrupt chan os.Signal, addr string, publicKey *ecdsa.PublicKey, signer crypto.Signer, networkID uint64, logger logging.Logger, libp2pPrivateKey, pssPrivateKey *ecdsa.PrivateKey, o *Options) (b *Bee, err error) {
	...
	syncRadius := newSyncRadius(o.FullNodeMode, swarmAddress, kad, batchStore)
	...
	pushSyncProtocol := pushsync.New(swarmAddress, blockHash, p2ps, storer, kad, syncRadius, tagService, o.FullNodeMode, pssService.TryUnwrap, validStamp, logger, acc, pricer, signer, tracer, warmupTime)

	// set the pushSyncer in the PSS
	pssService.SetPushSyncer(pushSyncProtocol)

	pusherService := pusher.New(networkID, storer, syncRadius, pushSyncProtocol, validStamp, tagService, logger, tracer, warmupTime)
	...
	if o.FullNodeMode && !o.BootnodeMode {
		pullerService = puller.New(stateStore, kad, syncRadius, pullSyncProtocol, logger, puller.Options{}, warmupTime)
