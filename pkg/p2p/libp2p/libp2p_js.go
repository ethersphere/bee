...
// New constructs a libp2p Service for wasm/browser builds. Unlike the native
// build, there are no TCP/WS listeners, no AutoTLS, and no AutoNAT: the browser
// can only dial out, and it does so over the wasmws (browser WebSocket)
// transport.
func New(ctx context.Context, signer beecrypto.Signer, networkID uint64, overlay swarm.Address, addr string, ab addressbook.GetPutter, storer storage.StateStorer, lightNodes *lightnode.Container, logger log.Logger, tracer *tracing.Tracer, o Options) (s *Service, returnErr error) {
	...
	// In the browser the only available transport is the WebSocket transport
	// backed by the host's WebSocket API.
	...
	s = &Service{
		...
		zapLogger:          nil,
		// The wasmws transport dials both ws and wss (the browser performs TLS),
		// so both are enabled for dial-filtering; only raw TCP is unavailable.
		enabledTransports: map[bzz.TransportType]bool{
			bzz.TransportTCP: false,
			bzz.TransportWS:  true,
			bzz.TransportWSS: true,
		},
