...
func TestHandshake(t *testing.T) {
	...
	parser := &mockParser{
		addr: node2BzzAddress,
	}

	handshakeService, err := handshake.New(signer1, aaddresser, senderMatcher, parser, node1Info.BzzAddress.Overlay, networkID, true, trxHash, testWelcomeMessage, node1AddrInfo.ID, logger)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("Handshake - picker error", func(t *testing.T) {
		...
		handshakeService, err := handshake.New(signer1, aaddresser, senderMatcher, nil, node1Info.BzzAddress.Overlay, networkID, true, trxHash, "", node1AddrInfo.ID, logger)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Handshake - welcome message too long", func(t *testing.T) {
		...
		_, err := handshake.New(signer1, aaddresser, senderMatcher, nil, node1Info.BzzAddress.Overlay, networkID, true, nil, LongMessage, node1AddrInfo.ID, logger)
	...
	t.Run("Handle - read error ", func(t *testing.T) {
		handshakeService, err := handshake.New(signer1, aaddresser, senderMatcher, nil, node1Info.BzzAddress.Overlay, networkID, true, nil, "", node1AddrInfo.ID, logger)
		if err != nil {
			t.Fatal(err)
		}
		testErr := errors.New("test error")
	...
	t.Run("Handle - transaction is not on the blockchain", func(t *testing.T) {
		...
		handshakeService, err := handshake.New(signer1, aaddresser, sbMock, nil, node1Info.BzzAddress.Overlay, networkID, true, trxHash, "", node1AddrInfo.ID, logger)
		if err != nil {
			t.Fatal(err)
		}
		var buffer1 bytes.Buffer
...
func (m *mockParser) ParseAck(ack *pb.Ack, blockHash []byte) (*bzz.Address, error) {
	return m.addr, m.err
}
