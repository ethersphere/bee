...
// StartFollowerSpan starts a new tracing span that is either a root one or a
// follower of existing one from the provided Context. If logger is provided, a new
// log Entry will be returned with "traceID" log field.
func (t *Tracer) StartFollowerSpan(ctx context.Context, operationName string, l log.Logger, opts ...opentracing.StartSpanOption) (opentracing.Span, log.Logger, context.Context) {
	if t == nil {
		t = noopTracer
	}
