...
func TestDelivery(t *testing.T) {
	...
	server := retrieval.New(
		swarm.MustParseHexAddress("0034"),
		mockStorer,
		nil, nil,
		logger,
...
func TestWaitForInflight(t *testing.T) {
	...
	// create the server that will handle the request and will serve the response
	server := retrieval.New(serverAddr, mockStorer, nil, nil, logger, serverMockAccounting, pricerMock, nil, false)

	badServer := retrieval.New(badServerAddr, badMockStorer, nil, nil, logger, badServerMockAccounting, pricerMock, nil, false)
	...
	client := retrieval.New(clientAddr, clientMockStorer, recorder, mt, logger, clientMockAccounting, pricerMock, nil, false)
...
func TestRetrieveChunk(t *testing.T) {
	...
	t.Run("downstream", func(t *testing.T) {
		...
		server := retrieval.New(serverAddress, serverStorer, nil, nil, logger, accountingmock.NewAccounting(), pricer, nil, false)
		...
		client := retrieval.New(clientAddress, nil, recorder, mt, logger, accountingmock.NewAccounting(), pricer, nil, false)
	...
	t.Run("forward", func(t *testing.T) {
		...
		client := retrieval.New(
			clientAddress,
			&testStorer{ChunkStore: inmemchunkstore.New()},                 // no chunk in clients's store
...
func TestRetrievePreemptiveRetry(t *testing.T) {
	...
	server1 := retrieval.New(serverAddress1, serverStorer1, nil, noClosestPeer, logger, accountingmock.NewAccounting(), pricerMock, nil, false)
	server2 := retrieval.New(serverAddress2, serverStorer2, nil, noClosestPeer, logger, accountingmock.NewAccounting(), pricerMock, nil, false)
	...
	t.Run("peer not reachable", func(t *testing.T) {
		...
		client := retrieval.New(clientAddress, nil, recorder, closetPeers, logger, accountingmock.NewAccounting(), pricerMock, nil, false)
	...
	t.Run("one peer is slower", func(t *testing.T) {
		...
		server1 := retrieval.New(serverAddress1, serverStorer1, nil, noClosestPeer, logger, server1MockAccounting, pricerMock, nil, false)
		server2 := retrieval.New(serverAddress2, serverStorer2, nil, noClosestPeer, logger, server2MockAccounting, pricerMock, nil, false)
		...
		client := retrieval.New(clientAddress, nil, recorder, closetPeers, logger, clientMockAccounting, pricerMock, nil, false)
	...
	t.Run("peer forwards request", func(t *testing.T) {
		...
		server1 := retrieval.New(
			serverAddress1,
			serverStorer1,
			server1Recorder,
			topologymock.NewTopologyDriver(topologymock.WithPeers(serverAddress2)),
			logger,
			accountingmock.NewAccounting(),
			pricerMock,
			nil,
		...
		client := retrieval.New(
			...
			nil,
			clientRecorder,
			topologymock.NewTopologyDriver(topologymock.WithPeers(serverAddress1)),
			logger,
			accountingmock.NewAccounting(),
			pricerMock,
			nil,
...
func TestClosestPeer(t *testing.T) {
	...
	ret := retrieval.New(srvAd, nil, nil, topologymock.NewTopologyDriver(topologymock.WithPeers(addr1, addr2, addr3)), log.Noop, nil, nil, nil, false)
