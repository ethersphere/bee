...
func newTestServer(t *testing.T, o testServerOptions) *testServer {
	...
	s := debugapi.New(o.PublicKey, o.PSSPublicKey, o.EthereumAddress, logging.New(ioutil.Discard, 0), nil, o.CORSAllowedOrigins, 2, transaction)
...
func TestServer_Configure(t *testing.T) {
	...
	ln := lightnode.NewContainer(o.Overlay)
	transaction := transactionmock.New(o.TransactionOpts...)
	s := debugapi.New(o.PublicKey, o.PSSPublicKey, o.EthereumAddress, logging.New(ioutil.Discard, 0), nil, nil, 2, transaction)
	ts := httptest.NewServer(s)
	t.Cleanup(ts.Close)
