...
func newTestServer(t *testing.T, o testServerOptions) *testServer {
	...
	s := debugapi.New(o.PublicKey, o.PSSPublicKey, o.EthereumAddress, logging.New(io.Discard, 0), nil, o.CORSAllowedOrigins, big.NewInt(2), transaction, false, debugapi.FullMode)
...
func TestServer_Configure(t *testing.T) {
	...
	ln := lightnode.NewContainer(o.Overlay)
	transaction := transactionmock.New(o.TransactionOpts...)
	s := debugapi.New(o.PublicKey, o.PSSPublicKey, o.EthereumAddress, logging.New(io.Discard, 0), nil, nil, big.NewInt(2), transaction, false, debugapi.FullMode)
	ts := httptest.NewServer(s)
	t.Cleanup(ts.Close)
