...
func TestSettlements(t *testing.T) {
	...
	expected := &debugapi.SettlementsResponse{
		TotalSettlementReceived: bigint.NewBigInt(15000),
		TotalSettlementSent:     bigint.NewBigInt(80000),
		Settlements: []debugapi.SettlementResponse{
			{
				Peer:               "DEAD",
				SettlementReceived: bigint.NewBigInt(0),
				SettlementSent:     bigint.NewBigInt(10000),
			},
			{
				Peer:               "BEEF",
				SettlementReceived: bigint.NewBigInt(10000),
				SettlementSent:     bigint.NewBigInt(20000),
			},
			{
				Peer:               "FFFF",
				SettlementReceived: bigint.NewBigInt(0),
				SettlementSent:     bigint.NewBigInt(50000),
			},
			{
				Peer:               "EEEE",
				SettlementReceived: bigint.NewBigInt(5000),
				SettlementSent:     bigint.NewBigInt(0),
			},
		},
	}
...
func TestSettlementsPeers(t *testing.T) {
	...
	jsonhttptest.Request(t, testServer.Client, http.MethodGet, "/settlements/"+peer, http.StatusOK,
		jsonhttptest.WithExpectedJSONResponse(debugapi.SettlementResponse{
			Peer:               peer,
			SettlementSent:     bigint.NewBigInt(1000000000000000000),
			SettlementReceived: bigint.NewBigInt(0),
		}),
	)
}
	...
	t.Run("no sent", func(t *testing.T) {
		...
		jsonhttptest.Request(t, testServer.Client, http.MethodGet, "/settlements/"+peer, http.StatusOK,
			jsonhttptest.WithExpectedJSONResponse(debugapi.SettlementResponse{
				Peer:               peer,
				SettlementSent:     bigint.NewBigInt(0),
				SettlementReceived: bigint.NewBigInt(1000000000000000000),
			}),
		)
	})
