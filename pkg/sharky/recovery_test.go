...
func TestMissingShard(t *testing.T) {
	_, err := sharky.NewRecovery(t.TempDir(), 1, 8, 8)
...
func XTestRecovery(t *testing.T) {
	...
	shards := 8
	shardSize := 16
	limitInChunks := shards * shardSize
...
func newSharky(t *testing.T, dir string, shards, shardSize, datasize int) *sharky.Store {
	t.Helper()
