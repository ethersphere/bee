...
type slots struct {
	...
	closed chan struct{} // indicate clean termination of process loop
}

func newSlots(file sharkyFile, shardSize int) *slots {
	return &slots{
		limit:  uint32(shardSize),
		file:   file,
		in:     make(chan uint32, releaseBufSize),
