...
type slots struct {
	...
	restore chan uint32 // outgoing channel for free slots
}
...
// extentions can only be multiples of 8
func (sl *slots) extend(n int) {
	// fmt.Printf("size: %d, limit: %d, head: %d, data: %x\n", sl.size, sl.limit, sl.head, sl.data)
	sl.size += uint32(n) * 8
	if sl.size > sl.limit {
		sl.size = sl.limit
	}
	for i := len(sl.data); i < int(sl.size); i += 8 {
		sl.data = append(sl.data, 0xff)
	}
}

// next returns the lowest free offset after start
