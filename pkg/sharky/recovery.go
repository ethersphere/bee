...
func NewRecovery(dir string, shardCnt int, datasize int) (*Recovery, error) {
	...
	shardFiles := make([]fs.OsFile, shardCnt)

	for i := range shardCnt {
		file, err := fs.OpenFile(path.Join(dir, fmt.Sprintf("shard_%03d", i)), os.O_RDWR, 0o666)
		if errors.Is(err, nativeFs.ErrNotExist) {
		...
		ffile, err := fs.OpenFile(path.Join(dir, fmt.Sprintf("free_%03d", i)), os.O_RDWR|os.O_CREATE, 0o666)
		if err != nil {
			return nil, err
		}
