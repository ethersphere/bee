...
import (
	...
	"github.com/ethersphere/bee/pkg/topology/kademlia/internal/retry"
...
func (k *Kad) Disconnected(peer p2p.Peer) {
	...
	failedAttemts := k.waitNext.Attempts(peer.Address)
	k.waitNext.Set(peer.Address, time.Now().Add(timeToRetry), failedAttemts)
