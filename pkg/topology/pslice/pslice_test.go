...
func TestAddRemove(t *testing.T) {
	...
	for i := 0; i < 8; i += 2 {
		a := swarm.RandAddressAt(t, base, i/2)
		peers[i] = a

		b := swarm.RandAddressAt(t, base, i/2)
		peers[i+1] = b
	}
...
func BenchmarkAdd(b *testing.B) {
	...
	for i := 0; i < bins*perBin; i++ {
		addrs = append(addrs, swarm.RandAddress(b))
	}
