...
// resolution represents monotonic sequence resolution.
// It must be in the time.Duration base form without a multiplier.
var resolution = time.Second
...
func New(dis p2p.Blocklister, flagTimeout, blockDuration, wakeUpTime time.Duration, callback func(swarm.Address), logger logging.Logger) *Blocker {
	if flagTimeout <= resolution {
		panic(fmt.Errorf("flag timeout %v cannot be equal to or lower then the clock resolution %v", flagTimeout, resolution))
	}
	if wakeUpTime < resolution {
		panic(fmt.Errorf("wakeup time %v cannot be lower then the clock resolution %v", wakeUpTime, resolution))
	}
...
func (b *Blocker) Flag(addr swarm.Address) {
	...
	if _, ok := b.peers[addr.ByteString()]; !ok {
		b.peers[addr.ByteString()] = &peer{
			blockAfter: b.sequence.Load() + uint64(b.flagTimeout/resolution),
			address:    addr,
		}
	}
}
