...
type AddressBook interface {
	...
	GetPeer(overlay []byte) ([]byte, []byte)
...
//	lock      sync.Mutex // guards seen
//	po        uint32
//	overlay   []byte
//	underlay  []byte
//}
//
//func (p *Peer) seen(peer *Peer) (seen bool) {
//	p.lock.Lock()
//	defer p.lock.Unlock()
//	_, ok := p.seenPeers[string(peer.overlay)]
//	p.seenPeers[string(peer.overlay)] = true
//	return ok
//}
...
func (s *Service) Init(ctx context.Context, peer p2p.Peer) error {
	...
	for _, address := range peers.BzzAddress {
		...
		if err := s.notifyPeerAdded(address.Overlay, address.Underlay); err != nil {
			return err
		}
	}

	//todo optional: do a 3-way style handshake here to make sure that both sides subscribed,
	// similar to handshake protocol
	return nil
}
...
func (s *Service) notifyDepthChanged(depth uint8) error {
	...
	for _, peer := range peers {
		...
		eg.Go(func() error {
			fmt.Println(peer)
			//		return s.sendSubscribe(context.Background(), peer, depth)
			return nil
		})
	}
...
func (s *Service) notifyPeerAdded(overlay, underlay []byte) error {
	//peers := s.addressBook.ConnectedPeers()
	//
	//var eg errgroup.Group
	//for _, p := range peers {
	//	peer := p
	//	eg.Go(func() error {
	//		return s.sendPeer(p, overlay, underlay)
	//	})
	//}
