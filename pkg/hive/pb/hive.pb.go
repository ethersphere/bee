...
type BzzAddress struct {
	...
	RTCUnderlays [][]byte `protobuf:"bytes,5,rep,name=RTCUnderlays,proto3" json:"RTCUnderlays,omitempty"`
}
...
func (m *BzzAddress) GetRTCUnderlays() [][]byte {
	if m != nil {
		return m.RTCUnderlays
	}
	return nil
}
...
var fileDescriptor_d635d1ead41ba02c = []byte{
	...
	0x88, 0xb4, 0xd2, 0x2c, 0x46, 0x2e, 0x2e, 0x84, 0xa8, 0x90, 0x14, 0x17, 0x47, 0x68, 0x5e, 0x4a,
	...
	0x56, 0xbf, 0xfc, 0xbc, 0xe4, 0x54, 0x09, 0x16, 0xb0, 0x38, 0x84, 0x23, 0xa4, 0xc4, 0xc5, 0x13,
	0x14, 0xe2, 0x0c, 0x33, 0xbc, 0x58, 0x82, 0x55, 0x81, 0x59, 0x83, 0x27, 0x08, 0x45, 0xcc, 0x49,
	...
	0xc0, 0x1e, 0x37, 0x06, 0x04, 0x00, 0x00, 0xff, 0xff, 0x5a, 0x9d, 0xa3, 0xbe, 0x06, 0x01, 0x00,
	0x00,
}
...
func (m *BzzAddress) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.RTCUnderlays) > 0 {
		for iNdEx := len(m.RTCUnderlays) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.RTCUnderlays[iNdEx])
			copy(dAtA[i:], m.RTCUnderlays[iNdEx])
			i = encodeVarintHive(dAtA, i, uint64(len(m.RTCUnderlays[iNdEx])))
			i--
			dAtA[i] = 0x2a
		}
	}
...
func (m *BzzAddress) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field RTCUnderlays", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.RTCUnderlays = append(m.RTCUnderlays, make([]byte, postIndex-iNdEx))
			copy(m.RTCUnderlays[len(m.RTCUnderlays)-1], dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipHive(dAtA[iNdEx:])
