...
func New(address swarm.Address, blockHash []byte, streamer p2p.StreamerDisconnecter, storer storage.Putter, topology topology.EachPeerer, syncRadius topology.SyncRadius, tagger *tags.Tags, isFullNode bool, unwrap func(swarm.Chunk), validStamp postage.ValidStampFn, logger logging.Logger, accounting accounting.Interface, pricer pricer.Interface, signer crypto.Signer, tracer *tracing.Tracer, warmupTime time.Duration) *PushSync {
	ps := &PushSync{
		address:        address,
		blockHash:      blockHash,
		streamer:       streamer,
...
func (ps *PushSync) handler(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	...
	storerNode := false
	defer func() {
		if !storerNode && ps.warmedUp() && ps.syncRadius.IsWithinSyncRadius(chunkAddress) {
		...
		if err != nil {
			...
			if errors.Is(err, topology.ErrWantSelf) {
				...
				if skipPeers.OverdraftListEmpty() { // no peers in skip list means we can be confident that we are the closest peer
					...
					if !ps.syncRadius.IsWithinSyncRadius(ch.Address()) {
