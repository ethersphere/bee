		...
		case r := <-resultC:
			...
			if r.err != nil && r.attempted {
				...
				if ps.topologyDriver.IsWithinDepth(peer) && !ps.skipList.hasChunk(ch.Address()) {
					ps.skipList.add(peer, ch.Address())
				}
			}
		case <-ctx.Done():
			return nil, ctx.Err()
...
func (l *peerSkipList) add(peer swarm.Address, chunk swarm.Address) {
	l.Lock()
	defer l.Unlock()

	l.skipExpiration[peer.ByteString()] = time.Now().Add(skipPeerExpiration)
