...
func TestUint64Field(t *testing.T) {
	...
	counter, err := db.NewUint64Field(ctx, "counter")
	if err != nil {
		t.Fatal(err)
	}
...
func TestUint64Field_IncInBatch(t *testing.T) {
	...
	batch := db.Store.GetBatch(true)
	var want uint64 = 1
	got, err := counter.IncInBatch(ctx, batch)
	if err != nil {
		t.Fatal(err)
	}
	if got != want {
	...
	want = 2
	got, err = counter.IncInBatch(ctx, batch2)
	if err != nil {
		t.Fatal(err)
	}
	if got != want {
...
func TestUint64Field_DecInBatch(t *testing.T) {
	...
	batch := db.Store.GetBatch(true)
	var want uint64
	got, err := counter.DecInBatch(ctx, batch)
	if err != nil {
		t.Fatal(err)
	}
	if got != want {
	...
	want = 41
	got, err = counter.DecInBatch(ctx, batch3)
	if err != nil {
		t.Fatal(err)
	}
	if got != want {
