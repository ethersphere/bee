...
// Index represents a set of BadgerDB key value pairs that have common
...
// BadgerDB keys and values for a specific index.
...
func (db *DB) NewIndex(ctx context.Context, name string, funcs IndexFuncs) (f Index, err error) {
	id, err := db.schemaIndexPrefix(ctx, name)
	if err != nil {
		return f, err
	}
	prefix := []byte{id}
	return Index{
		db:     db,
		prefix: prefix,
		// This function adjusts Index BadgerDB key
...
func (f Index) Get(ctx context.Context, keyFields Item) (out Item, err error) {
	key, err := f.encodeKeyFunc(keyFields)
	if err != nil {
		return out, err
	}
	value, err := f.db.Store.Get(ctx, key)
	if err != nil {
		return out, err
	}
...
func (f Index) Fill(ctx context.Context, items []Item) (err error) {
	txn := f.db.Store.GetBatch(false)
	defer txn.Discard()
...
// the index.
func (f Index) Has(ctx context.Context, keyFields Item) (bool, error) {
	key, err := f.encodeKeyFunc(keyFields)
	if err != nil {
		return false, err
	}
	return f.db.Store.Has(ctx, key)
}
...
func (f Index) Put(ctx context.Context, i Item) (err error) {
	...
	return f.db.Store.Put(ctx, key, value)
}
...
func (f Index) Delete(ctx context.Context, keyFields Item) (err error) {
	key, err := f.encodeKeyFunc(keyFields)
	if err != nil {
		return err
	}
	return f.db.Store.Delete(ctx, key)
}
...
func (f Index) Iterate(fn IndexIterFunc, options *IterateOptions) (err error) {
	...
	err = f.db.Store.Iterate(startKey, options.SkipStartFromItem, func(key []byte, value []byte) (stop bool, err error) {
...
// If Index has no elements, a storage.ErrNotFound error is returned.
func (f Index) First(prefix []byte) (i Item, err error) {
	totalPrefix := append(f.prefix, prefix...)
	k, v, err := f.db.Store.First(totalPrefix)
	if err != nil {
		return i, err
	}
...
// storage.ErrNotFound is returned. Value for totalPrefix must start with
// Index prefix.
...
func (f Index) Last(prefix []byte) (i Item, err error) {
	totalPrefix := append(f.prefix, prefix...)
	k, v, err := f.db.Store.Last(totalPrefix)
	if err != nil {
		return i, err
	}
	return f.itemFromKeyValue(k, v, totalPrefix)
...
func (f Index) Count() (count int, err error) {
	return f.db.Store.CountPrefix(f.prefix)
}
...
func (f Index) CountFrom(start Item) (count int, err error) {
	...
	return f.db.Store.CountFrom(startKey)
}
