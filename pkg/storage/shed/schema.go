...
func (db *DB) schemaFieldKey(ctx context.Context, name, fieldType string) (key []byte, err error) {
	if name == "" {
...
func (db *DB) RenameIndex(ctx context.Context, name, newName string) (renamed bool, err error) {
	if name == "" {
	...
	for i, f := range s.Indexes {
		if f.Name == name {
			...
			return true, db.putSchema(ctx, s)
		}
		if f.Name == newName {
			return true, nil
		}
...
// a particular index.
func (db *DB) schemaIndexPrefix(ctx context.Context, name string) (id byte, err error) {
	if name == "" {
	...
	return id, db.putSchema(ctx, s)
}

// getSchema retrieves the complete schema from
// the database.
func (db *DB) getSchema(ctx context.Context) (s schema, err error) {
	b, err := db.Store.Get(ctx, keySchema)
	if err != nil {
		return s, err
	}
...
// the database.
func (db *DB) putSchema(ctx context.Context, s schema) (err error) {
	b, err := json.Marshal(s)
	if err != nil {
		return err
	}
	return db.Store.Put(ctx, keySchema, b)
}
