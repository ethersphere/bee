...
import (
	...
	"github.com/ethersphere/bee/pkg/storage/shed"
...
func New(path string) (s *Store, err error) {
	...
	s.schemaName, err = db.NewStringField(ctx, "schema-name")
	if err != nil {
		return nil, err
	}
	// Global ever incrementing index of chunk accesses.
	s.accessCounter, err = db.NewUint64Field(ctx, "access-counter")
	if err != nil {
		return nil, err
	}
	// Index storing actual chunk address, data and store timestamp.
	s.retrievalIndex, err = db.NewIndex(ctx, "Address->StoreTimestamp|Data", shed.IndexFuncs{
	...
	s.accessIndex, err = db.NewIndex(ctx, "Address->AccessTimestamp", shed.IndexFuncs{
		EncodeKey: func(fields shed.Item) (key []byte, err error) {
			return fields.Address, nil
		},
		DecodeKey: func(key []byte) (e shed.Item, err error) {
	...
	s.gcIndex, err = db.NewIndex(ctx, "AccessTimestamp|StoredTimestamp|Address->nil", shed.IndexFuncs{
		EncodeKey: func(fields shed.Item) (key []byte, err error) {
...
func (s *Store) Put(ctx context.Context, key []byte, value []byte) (err error) {
	return s.retrievalIndex.Put(ctx, shed.Item{
		Address:        key,
		Data:           value,
...
// Get retrieves a value with the provided key.
...
func (s *Store) Get(ctx context.Context, key []byte) (value []byte, err error) {
	batch := s.db.Store.GetBatch(true)

	// Get the chunk data and storage timestamp.
	item, err := s.retrievalIndex.Get(ctx, shed.Item{
		Address: key,
	})
	if err != nil {
	...
	accessItem, err := s.accessIndex.Get(ctx, shed.Item{
		Address: key,
	})
	switch err {
	case nil:
	...
	_, err = s.accessCounter.IncInBatch(ctx, batch)
	if err != nil {
		return value, err
	}

	// Write the batch.
	err = s.db.Store.WriteBatch(batch)
	if err != nil {
		return value, err
	}
...
func (s *Store) CollectGarbage() (err error) {
	...
	for roundCount := 0; roundCount < maxRounds; roundCount++ {
		...
		trash := s.db.Store.GetBatch(true)
		...
		err = s.db.Store.WriteBatch(trash)
		if err != nil {
			return err
		}
	}
...
func (s *Store) GetSchema(ctx context.Context) (name string, err error) {
	name, err = s.schemaName.Get(ctx)
	if err == storage.ErrNotFound {
		return "", nil
	}
	return name, err
...
func (s *Store) PutSchema(ctx context.Context, name string) (err error) {
	return s.schemaName.Put(ctx, name)
}

// Close closes the underlying database.
func (s *Store) Close(ctx context.Context) error {
	return s.db.Store.Close(ctx)
}
...
func Example_store() {
	...
	ch := GenerateRandomChunk(1024)
	err = s.Put(context.Background(), ch.Address().Bytes(), ch.Data().Bytes())
	if err != nil {
		log.Fatal(err)
	}

	gotValue, err := s.Get(context.Background(), ch.Address().Bytes())
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println(bytes.Equal(gotValue, ch.Data().Bytes()))

	//Output: true
}

func GenerateRandomChunk(size int) swarm.Chunk {
	data := make([]byte, size)
	rand.Read(data)
