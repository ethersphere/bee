...
func (db *DB) NewStringField(ctx context.Context, name string) (f StringField, err error) {
	key, err := db.schemaFieldKey(ctx, name, "string")
	if err != nil {
		return f, err
	}
	return StringField{
...
// an no error.
func (f StringField) Get(ctx context.Context) (val string, err error) {
...
func (f StringField) Put(ctx context.Context, val string) (err error) {
	return f.db.Store.Put(ctx, f.key, []byte(val))
}
