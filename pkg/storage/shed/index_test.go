...
func TestIndex(t *testing.T) {
	...
	index, err := db.NewIndex(ctx, "retrieval", retrievalIndexFuncs)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("has", func(t *testing.T) {
		...
		has, err = index.Has(ctx, dontWant)
		if err != nil {
			t.Fatal(err)
		}
		if has {
...
func TestIndex_Iterate(t *testing.T) {
	...
	t.Run("no overflow", func(t *testing.T) {
		secondIndex, err := db.NewIndex(ctx, "second-index", retrievalIndexFuncs)
		if err != nil {
			t.Fatal(err)
		}
		...
		err = secondIndex.Put(ctx, secondItem)
		if err != nil {
			t.Fatal(err)
		}
