...
func (db *DB) NewUint64Field(ctx context.Context, name string) (f Uint64Field, err error) {
	key, err := db.schemaFieldKey(ctx, name, "uint64")
	if err != nil {
		return f, err
	}
	return Uint64Field{
...
func (f Uint64Field) Get(ctx context.Context) (val uint64, err error) {
...
func (f Uint64Field) Put(ctx context.Context, val uint64) (err error) {
	return f.db.Store.Put(ctx, f.key, encodeUint64(val))
}
...
func (f Uint64Field) Inc(ctx context.Context) (val uint64, err error) {
	val, err = f.Get(ctx)
	if err != nil {
		if err == storage.ErrNotFound {
			val = 0
	...
	val++
	return val, f.Put(ctx, val)
}

// IncInBatch increments a uint64 value in the batch
// by retreiving a value from the database, not the same batch.
// This operation is not goroutine save.
func (f Uint64Field) IncInBatch(ctx context.Context, batch *badger.Txn) (val uint64, err error) {
	val, err = f.Get(ctx)
	if err != nil {
		if err == storage.ErrNotFound {
			val = 0
...
func (f Uint64Field) Dec(ctx context.Context) (val uint64, err error) {
	val, err = f.Get(ctx)
	if err != nil {
		if err == storage.ErrNotFound {
			val = 0
...
// This operation is not goroutine save.
// The field is protected from overflow to a negative value.
func (f Uint64Field) DecInBatch(ctx context.Context, batch *badger.Txn) (val uint64, err error) {
	val, err = f.Get(ctx)
	if err != nil {
		if err == storage.ErrNotFound {
			val = 0
