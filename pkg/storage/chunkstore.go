...
// avoiding per-call allocations. If len(buf) is smaller than the stored chunk,
// GetInto returns an error and does not write to buf. Returns the number of
// bytes read; callers use buf[:n].
