...
func (c *command) configureSigner(cmd *cobra.Command, logger log.Logger) (config *signerConfig, err error) {
	...
	if c.config.GetBool(optionNameClefSignerEnable) {
		...
		swarmPrivateKey, _, err := keystore.Key("swarm", password, new(crypto.Secp256k1EDG))
		if err != nil {
	...
	libp2pPrivateKey, created, err := keystore.Key("libp2p", password, new(crypto.Secp256r1EDG))
	if err != nil {
		...
		libp2pPrivateKey, err = keystore.SetKey("libp2p", password, new(crypto.Secp256r1EDG))
		if err != nil {
	...
	pssPrivateKey, created, err := keystore.Key("pss", password, new(crypto.Secp256k1EDG))
	if err != nil {
