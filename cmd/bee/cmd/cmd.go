...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String(optionNamePaymentThreshold, "40000", "threshold in BZZ where you expect to get paid from your peers")
	cmd.Flags().String(optionNamePaymentTolerance, "40000", "excess debt above payment threshold in BZZ where you disconnect from your peer")
	cmd.Flags().String(optionNamePaymentEarly, "20000", "amount in BZZ below the peers payment threshold when we initiate settlement")
