...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Int(optionReserveCapacityDoubling, 0, "number of the times the reserve capacity is doubled eg: capacity * 2^d. Stake should be at least 2^h * 10 BZZ in order to fully parcipate in the storage incentives for all of the neighborhoods.")
}
