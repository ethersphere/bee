...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String(optionNamePaymentTolerance, "100000", "excess debt above payment threshold in BZZ where you disconnect from your peer")
	cmd.Flags().String(optionNamePaymentEarly, "50000", "amount in BZZ below the peers payment threshold when we initiate settlement")
